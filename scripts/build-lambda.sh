...
sed 's|require("../lib/|require("./lib/|g; s|require("../shared/|require("./shared/|g' .build/handlers/authorizer.js > .build/lambda-packages/authorizer/index.js
...
cp -r .build/lib .build/lambda-packages/authorizer/lib 2>/dev/null || true
...
cp -r .build/shared .build/lambda-packages/authorizer/shared 2>/dev/null || true
...
cp package.json .build/lambda-packages/authorizer/
...
cd ../authorizer && npm install --production --no-save --silent
cd ../../..
